...
module.exports = {
  ...
  async forceAccept (receivedMessage, args) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)

    if (args.length === 1) {
      const returnArr = await bootstrap.GameObj.forceAccept(args, receivedMessage.guild.id)
  ...
  async getDisputed (receivedMessage) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const returnArr = await bootstrap.GameObj.getPending(receivedMessage.guild.id, 'Disputed')
  ...
  async getPending (receivedMessage) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const returnArr = await bootstrap.GameObj.getPending(receivedMessage.guild.id)
    if (returnArr === 'No Pending') {
      const noPendingEmbed = new bootstrap.Discord.MessageEmbed()
        .setColor(bootstrap.messageColorBlue)
  ...
  async configGet (receivedMessage) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const returnArr = await bootstrap.LeagueObj.configGet(receivedMessage.guild.id)
  ...
  async configSet (receivedMessage, args) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const returnArr = await bootstrap.LeagueObj.configSet(receivedMessage, args)
  ...
  async setSeasonName (receivedMessage, args) {
    ...
    if (args[0] === undefined) {
      ...
      const returnArr = await bootstrap.SeasonObj.setSeasonName(receivedMessage, args)
  ...
  async setEndSeason (receivedMessage, args) {
    ...
    if (args[0].length === 10 && args.length === 1) {
      ...
      if (date instanceof Date && !isNaN(date.valueOf())) {
        ...
        const returnArr = await bootstrap.SeasonObj.setEndDate(receivedMessage, date)
  ...
  async seasonInfo (receivedMessage, args) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    if (args[0] === undefined) {
      const returnArr = await bootstrap.SeasonObj.getInfo(receivedMessage, 'Current')
      ...
      const returnArr = await bootstrap.SeasonObj.getInfo(receivedMessage, 'all')
      if (returnArr[0] !== '') {
      ...
      const returnArr = await bootstrap.SeasonObj.getInfo(receivedMessage, args.join(' '))
  ...
  async startSeason (receivedMessage) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const returnArr = await bootstrap.SeasonObj.startSeason(receivedMessage)
  ...
  async top (receivedMessage, args) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const returnArr = await bootstrap.SeasonObj.leaderBoard(receivedMessage)
    const mentionValues = []
    if (args.length === 0) {
    ...
    const lookUpUsers = await mentionValues.map(bootstrap.SeasonHelper.lookUpUsers)
    ...
    Promise.all(lookUpUsers).then(results => {
      for (let i = 0; i < results.length; i++) {
        if (results[i] !== "Can't find deck") {
          const calculatedWinrate = Math.round(results[i][0][1] / (results[i][0][1] + results[i][0][2]) * 100)
          const elo = (20 * (results[i][0][1])) - (10 * (results[i][0][2])) + 1000
          const username = results[i][0][0]
          const gamesPlayed = (results[i][0][1] + results[i][0][2])
      ...
      const getDeckThreshold = await bootstrap.ConfigHelper.getDeckThreshold(receivedMessage.guild.id)
      const sortedResults = unsortedResults
      let threshold = 5
      ...
      for (let i = 0; i < sortedResults.length; i++) {
        if (sortedResults[i][3] < threshold) { } else if (i > 7) { } else {
  ...
  async deckinfo (receivedMessage, args, rawArgs) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const returnArr = await bootstrap.DeckObj.deckInfo(receivedMessage, args)
    if (args.length === 0) {
      const errorEmbed = new bootstrap.Discord.MessageEmbed()
        .setColor(bootstrap.messageColorRed)
    ...
    if (returnArr === 'Error 1') {
      ...
      if (returnArr[0] === 'First') {
        const fixedColors = returnArr[1]._colors.replace(/,/g, ' ')
  ...
  async updateDeck (receivedMessage, args) {
    ...
    const promiseReturn = await bootstrap.DeckHelper.findDeckToUpdate(receivedMessage, args)
  ...
  async removeDeck (receivedMessage, args) {
    ...
    const promiseReturn = await bootstrap.DeckHelper.findDeckToRemove(receivedMessage, args)
    if (promiseReturn === 'Error 1') {
      addingDeckEmbed
        .setColor(bootstrap.messageColorRed) // red
        // eslint-disable-next-line no-multi-str
  ...
  async deckStats (receivedMessage, args) {
    ...
    const returnArr = await bootstrap.DeckObj.deckStats(receivedMessage, args)
    if (returnArr[0] === 'Deck Lookup') {
      const deckName = returnArr[1].split(' | ')
      let seasonName
      ...
      for (let i = 0; i < returnArr[5].length; i++) {
        usersList.addFields(
      ...
      const sortedArray = returnArr[1].sort(function (a, b) {
        return parseFloat(b[1] + b[2]) - parseFloat(a[1] + a[2])
      })
      sortedArray.forEach((deck) => {
        if (deck[1] + deck[2] < threshold) { } else {
          allDecksEmbed
            .addFields(
  ...
  async use (receivedMessage, args, rawArgs) {
    ...
    if (args.length === 0) {
      ...
      const returnArr = await bootstrap.UserObj.useDeck(receivedMessage, args)
      if (returnArr === 'Success') {
        const successEmbed = new bootstrap.Discord.MessageEmbed()
          .setColor(bootstrap.messageColorGreen)
          .setAuthor('Success')
  ...
  async recent (receivedMessage, args) {
    ...
    if (args.length === 0) {
      matches = await bootstrap.UserObj.recent(receivedMessage)
      ...
      if (args[0].toLowerCase() === 'more') {
        ...
        matches = await bootstrap.UserObj.recent(receivedMessage, null, true)
      } else {
        matches = await bootstrap.UserObj.recent(receivedMessage, args[0])
      }
    ...
    const confirmEmbed = new bootstrap.Discord.MessageEmbed()
      .setColor(bootstrap.messageColorGreen)
      .setDescription('Showing ' + matches.length.toString() + ' recent ' + matchGrammar)
    generalChannel.send(confirmEmbed)

    // Main loop
    let tempEmbed
    matches.forEach(async (match) => {
      const convertedToCentralTime = match[0].toLocaleString('en-US', { timeZone: 'America/Chicago' })
  ...
  async startMatch (receivedMessage, args) {
    const currentSeason = await bootstrap.SeasonHelper.getCurrentSeason(receivedMessage.guild.id)
    const generalChannel = bootstrap.Client.channels.cache.get(receivedMessage.channel.id)
    const sanitizedString = receivedMessage.author.id
    const UserIDs = []
    ...
    const registerPromiseArray = mentionValues.map(bootstrap.GameHelper.checkRegister)
    ...
    Promise.all(registerPromiseArray).then(results => {
      ...
      const currDeckPromiseArray = mentionValues.map(bootstrap.GameHelper.checkDeck)
                ...
                bootstrap.GameObj.createMatch(UserIDs[0], UserIDs[1], UserIDs[2], UserIDs[3], id, receivedMessage, function (cb) {
                  if (cb === 'FAILURE') {
                    ...
                    UserIDs.forEach(player => {
                      const findQuery = { _mentionValue: player, _server: receivedMessage.guild.id }
  ...
  async listDecks (receivedMessage, args) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    if (args[0] !== undefined) {
      const colorRes = await bootstrap.DeckHelper.checkColorDictionary(args[0])
      if (colorRes !== 'Not found') {
        const colorSpecificArray = []
        const returnArr = await bootstrap.DeckObj.listDecks(receivedMessage, colorRes)
      ...
      const commanderRes = await bootstrap.DeckHelper.commanderChecker(args.join(' '), receivedMessage)
    ...
    const returnArr = await bootstrap.DeckObj.listDecks(receivedMessage, 'no')
    const oneColorArr = []
    const twoColorArr = []
    const threeColorArr = []
    const fourColorArr = []
  ...
  async addDeck (receivedMessage, args) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const nicknameArr = args.slice(0, args.indexOf('|'))
    let deckNickname = ''
    ...
    if (splitArgs.length === 9) {
      const deckNick = deckNickname
      let commanderName = splitArgs[1]
      let colorIdentity = splitArgs[2].toLowerCase()
      let deckLink = splitArgs[3]
      const author = splitArgs[4]
      const deckDescription = splitArgs[5]
      const deckType = splitArgs[6]
      const hasPrimer = splitArgs[7]
      let discordLink = splitArgs[8]
      commanderName = commanderName.replace(/ {2}/g, ', ')
      ...
      const promiseReturn = await bootstrap.DeckObj.addDeck(receivedMessage, newDeckArr)
      if (promiseReturn === 'Error 1') {
  ...
  async profile (receivedMessage, args) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const returnArr = await bootstrap.UserObj.profile(receivedMessage, args)
    let compareDeck = 0
    let favDeck = ''
    let elo = 1000
    let overallWins = 0
  ...
  async remindMatch (receivedMessage, args) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const playerID = receivedMessage.author.id
    ...
    const response = await bootstrap.GameObj.getRemindInfo(playerID, receivedMessage.guild.id).catch((message) => {
      const errorMsg = new bootstrap.Discord.MessageEmbed()
        .setColor('#af0000')
  ...
  async matchInfo (receivedMessage, args) {
    ...
    if (response !== 'FAIL') {
      const convertedToCentralTime = response[0].toLocaleString('en-US', { timeZone: 'America/Chicago' })
      ...
      const tempEmbed = new bootstrap.Discord.MessageEmbed()
        .setColor(bootstrap.messageColorBlue) // blue
  ...
  async register (receivedMessage) {
    const generalChannel = bootstrap.MessageHelper.getChannelID(receivedMessage)
    const returnArr = await bootstrap.LeagueObj.register(receivedMessage)
  ...
  helpCommand (receivedMessage, args) {
    if (args.length === 0) {
      ...
      bootstrap.FunctionHelper.showEmbedHelpForCommand(receivedMessage, args)
    }
  },
