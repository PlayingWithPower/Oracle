...
module.exports = {
  /**
     ...
     * @param {*} args - command key
     *
     * Retrieves info from the help dictionary about a specific command.
     */
  showEmbedHelpForCommand (receivedMessage, args) {
    const exampleEmbed = new bootstrap.Discord.MessageEmbed()
      .setColor(bootstrap.messageColorBlue)
    exampleEmbed
      .setAuthor('Displaying information about the command: !' + [args])
    if (exampleDictionary[args] !== undefined) {
      exampleEmbed
        .addFields(
          { name: 'Command Details', value: exampleDictionary[args] }
        )
    } else {
  ...
  async showEmbedHelpForAllCommands (receivedMessage) {
    ...
    for (const keyVal in adminDictionary) {
      adminEmbed.addField('!' + keyVal, adminDictionary[keyVal])
    }
    for (const keyVal in deckDictionary) {
      deckEmbed.addField('!' + keyVal, deckDictionary[keyVal])
    }
    for (const keyVal in gameDictionary) {
      gameEmbed.addField('!' + keyVal, gameDictionary[keyVal])
    }
    for (const keyVal in leagueDictionary) {
      leagueEmbed.addField('!' + keyVal, leagueDictionary[keyVal])
    }
    for (const keyVal in seasonDictionary) {
      seasonEmbed.addField('!' + keyVal, seasonDictionary[keyVal])
    }
    for (const keyVal in userDictionary) {
      userEmbed.addField('!' + keyVal, userDictionary[keyVal])
    }

    receivedMessage.author.send(deckEmbed)
      .then(msg => {
        receivedMessage.author.send(gameEmbed)
          .then(msg => {
            receivedMessage.author.send(leagueEmbed)
              .then(msg => {
                receivedMessage.author.send(seasonEmbed)
                  .then(msg => {
                    receivedMessage.author.send(userEmbed).then(async msg => {
